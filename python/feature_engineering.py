...
def get_auc(df,  scaler=None, target="quality", cols_adj=cols_to_adjust, n_iters=1, split=0.4):
    ...
    for seed in range(n_iters):
        ...
        if scaler is not None:
            df_transform = ColumnTransformer([(" ", scaler, cols_adj)], remainder="passthrough")
...
robscale_results = get_auc(df_train_raw, preprocessing.RobustScaler(quantile_range=(.2, .8))) # Drops outliers

pwr_transform_results = get_auc(df_train_raw, preprocessing.PowerTransformer()) #  yao johnson

quant_transfmr_results = get_auc(df_train_raw, preprocessing.QuantileTransformer()) # transforms data to unf(0,1)
...
results = pd.DataFrame(res_zip, 
        columns=["baseline_results", "std_scale_results", "min_max_results", "robscale_results", "pwr_transform_results", "quant_transfmr_results"])\
