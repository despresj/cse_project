...
df_raw = df_raw.groupby(df_raw.columns.tolist()).size()\
    .reset_index()\
    .rename(columns={"size": "duplicates"})
y = df_raw.quality
...
rescaled = scaler_min_max.transform(inputed)

X_train = np.concatenate((rescaled, already_scaled), axis=1)
...
test_rescaled = scaler_min_max.transform(test_inputed)

X_test = np.concatenate((test_rescaled, test_scaled), axis=1)
np.savez(
    "data/processed/train_test_scale_1.npz",
    X_train=X_train,
    X_test=X_test,
    y_train=y_train,
    y_test=y_test,
